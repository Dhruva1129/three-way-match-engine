...
export function FilePreview({ documentId }: { documentId: string | null }) {
  ...
  return (
    <div className="panel flex h-full min-h-0 flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-hidden bg-slate-100 p-3">
        {error && (
        ...
        {state && (
          <div style={{ width: `${zoom}%`, transition: "width 0.15s ease" }} className="mx-auto flex h-full min-h-0 flex-col">
            {state.mimeType.startsWith("image/") ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={state.url} alt="Document preview" className="h-full w-full rounded border border-slate-200 bg-white object-contain shadow-sm" />
            ) : (
              <iframe src={state.url} title="Document preview" className="h-full w-full rounded border border-slate-200 bg-white shadow-sm" />
            )}
          </div>
        )}
      </div>
